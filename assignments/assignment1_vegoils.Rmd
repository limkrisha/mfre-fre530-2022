...
# Testing for Stationarity (8 points)
  ...
  a) Perform a Dickey-Fuller (DF) test (i.e., no constant and no lags) to determine whether each price series is stationary or not. Be clear and concise in explaining how you concluded whether the price series is stationary or not. *Hint: You will do 3 separate DF tests.* (1 point)
  b) In recent years, an Augmented Dickey Fuller (ADF) test has been developed to account for potential autocorrelation in the residuals. (5 points) 
    - Without adding any drift or trend terms, add lags to your stationary test for soybeans. Briefly explain how you determined the number of lags to include in your model. Discuss the results in 2-3 sentences.
    - Add an intercept term and lags to the ADF test. Discuss the results in 2-3 sentences. 
    - Add an intercept, trend, and lags to the ADF test. Discuss the results in 2-3 sentences.
    - Based on the three ADF tests you have performed, which specification would you suggest to use? Why?
  c) Perform an ADF test whether the first difference of each price series is stationary or not. (1 point)
...
**\textcolor{blue}{Suggested Answers}**
  ...
  - $DF_{palm}$: t-statistic is `r round(df_palm@teststat, 4)` and the critical values are `r df_palm@cval`. We fail to reject the null hypothesis of a unit root. Palm price is not stationary. 
  - $DF_{soy}$: t-statistic is `r round(df_soy@teststat, 4)` and the critical values are `r df_soy@cval`. We fail to reject the null hypothesis of a unit root. Soybean oil price is not stationary. 
  - $DF_{rapeseed}$: t-statistic is `r round(df_rapeseed@teststat, 4)` and the critical values are `r df_rapeseed@cval`. We fail to reject the null hypothesis of a unit root. Rapeseed price is not stationary. 
    
    
  b) ADF tests: 
  
  - Visual inspection of the partial autocorrelation plot suggests we include two lags. Only 1 lag is included in the final specification if we select lags based on the lowest AIC or BIC. With the specification `lags = 2, selectlags = c("AIC")`, the t-statistic is `r round(adf_soy@teststat, 4)` and the critical values are `r adf_soy@cval`. We fail to reject the null hypothesis. Soy price is not stationary, even after adding controls for autocorrelation. 
  - By specifying `type = c("drift")` we add a intercept to the model. The $\phi_1$ t-statistic is `r round(adf_soy_drift@teststat[2], 4)` and the critical values are `r adf_soy_drift@cval[2,]`. We reject the joint null hypothesis, so there is a unit root and we can exclude the drift term. The $\tau_2$ t-statistic is `r round(adf_soy_drift@teststat[1], 4)` and the critical values are `r adf_soy_drift@cval[1,]`, so there is a unit root.
  - By specifying `type = c("trend")` we add a drift and an intercept to the model. The $\phi_2$ t-statistic is `r round(adf_soy_trend@teststat[2], 4)` and the critical values are `r adf_soy_trend@cval[2,]`. We reject the joint null hypothesis, so there is a unit root and we can exclude the drift term and trend terms. The $\phi_3$ t-statistic is `r round(adf_soy_trend@teststat[3], 4)` and the critical values are `r adf_soy_trend@cval[3,]`; it shows that the data is stationary without a drift. The $\tau_3$ t-statistic is `r round(adf_soy_trend@teststat[1], 4)` and the critical value are `r adf_soy_trend@cval[1,]` shows that there is a unit root. 
  - Based on these different ADF specifications, we should include a lag term to control for autocorrelation. Trend and drift terms are not necessary in the ADF test for soybean oil. 
...
To test whether palm oil, soybean oil, and rapeseed oil are cointegrated, we should ideally use the Johansen test for cointegration. However, because the Johansen procedure is relatively complicated, we will instead test all three price pairs for cointegration with the Engle-Granger procedure. If all three pairs are cointegrated, we will assume that the three prices are jointly cointegrated. 
