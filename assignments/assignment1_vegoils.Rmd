...
# saveRDS(cmo, here("assignments", data", "vegoils.RDS"))
...
**\textcolor{blue}{Suggested Answers}**
  ...
  - By specifying `type = c("trend")` we add a drift and trend term to the model. The $\phi_2$ t-statistic is `r round(adf_soy_trend@teststat[2],3)` and the critical values are `r adf_soy_trend@cval[2,]`, so we fail to reject the joint null hypothesis; there is a unit root and we can exclude the drift and trend terms. The $\phi_3$ t-statistic is `r round(adf_soy_trend@teststat[3],3)` and the critical values are `r adf_soy_trend@cval[3,]`, indicating we fail to reject the joint null hypothesis; there is a unit root but no drift term. The $\tau_3$ t-statistic is `r round(adf_soy_trend@teststat[1],3)` and the critical values are `r adf_soy_trend@cval[1,]`, so we fail to reject the null hypothesis of a unit root. According to the Stationarity Testing Flow Chart posted on Canvas, soybean oil price has a unit root without drift. 
  ...
  c) In the ADF test of the first difference of palm oil, soybean oil, and rapeseed oil, the $\tau_3$ test statistics are `r round(adf_palm_diff@teststat[1],4)`, `r round(adf_soy_diff@teststat[1],4)`, `r round(adf_rapeseed_diff@teststat[1],4)`, respectively. The critical values are `r adf_soy_diff@cval[1,]`. We reject the null hypothesis of a unit root for each of the price series; drift and trend terms must be included in the model too. We therefore conclude that each price series is integrated of the order 1. 
